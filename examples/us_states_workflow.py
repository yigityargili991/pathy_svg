...
density = {st: population[st] / land_area[st] for st in population if st in land_area}

# ── 1. Population heatmap ────────────────────────────────────────────

doc = SVGDocument.from_file(SVG_PATH)

pop_map = doc.heatmap(population, palette="YlOrRd").legend(
    title="Population", position=(0.85, 0.1), size=(0.03, 0.4)
)
...
density_map = doc.heatmap(density, palette="viridis", vmax=1500).legend(
    title="People / sq mi", position=(0.85, 0.1), size=(0.03, 0.4)
)
...
highlighted = doc.heatmap(population, palette="YlOrRd").highlight(
    top10, dim_opacity=0.15, desaturate=True
)
...
for st, d in density.items():
    if d > median_density * 2:
        patterns[st] = PatternSpec(
            kind="dots", color="#b30000", spacing=5, thickness=1.5
        )
...
stroked = doc.heatmap(density, palette="YlGnBu", vmax=1500).stroke_map(
    population, width_range=(0.5, 5.0), palette="Greys"
)
...
layered = (
    ...
    .flatten()
    .legend(title="Density (people/sq mi)", position=(0.85, 0.1), size=(0.03, 0.4))
)
