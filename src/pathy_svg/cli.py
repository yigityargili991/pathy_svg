...
def heatmap(svg_file, data_file, id_col, value_col, palette, legend, legend_title, output):
...
def validate(svg_file, data_file, id_col):
    ...
    if result.unmatched:
        click.echo(f"Unmatched ({len(result.unmatched)}): {', '.join(result.unmatched)}")
    if result.unused:
        click.echo(f"Unused SVG IDs ({len(result.unused)}): {', '.join(result.unused[:10])}")
    if result.is_valid:
...
@main.command()
@click.argument("svg_file", type=click.Path(exists=True))
@click.option("--format", "fmt", type=click.Choice(["png", "pdf", "jpeg"]), default="png")
...
@click.option("--id-col", required=True, help="Column name for path IDs")
@click.option("--value-col", required=True, help="Column name for values")
@click.option("--mode", type=click.Choice(["delta", "ratio", "log2ratio", "percent_change"]), default="delta")
@click.option("--palette", default="coolwarm")
@click.option("-o", "--output", required=True, help="Output SVG path")
def diff(svg_file, baseline_file, treatment_file, id_col, value_col, mode, palette, output):
