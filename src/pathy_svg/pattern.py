...
from pathy_svg._constants import COLORABLE_TAGS, get_secure_parser, SVG_NS, build_id_index, local_tag, safe_svg_id, svg_sub
...
def _validate_pattern_spec(pat_id: str, spec: PatternSpec) -> None:
    ...
    if isinstance(spec, CustomPatternSpec):
        try:
            etree.fromstring(f"<wrapper xmlns='{SVG_NS}'>{spec.markup}</wrapper>", parser=get_secure_parser())
...
def _build_custom_pattern(
    ...
    fragment = etree.fromstring(f"<wrapper xmlns='{SVG_NS}'>{spec.markup}</wrapper>", parser=get_secure_parser())
